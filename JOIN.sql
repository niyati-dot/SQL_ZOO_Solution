...
SELECT g.player, g.teamid, go.coach, g.gtime
 FROM goal g
 JOIN eteam go ON g.teamid = go.id
 WHERE g.gtime <= 10;
...
SELECT e.teamname, COUNT(g.teamid) AS 'Goal Scored'
 FROM eteam e
 LEFT JOIN goal g ON e.id = g.teamid
 GROUP BY teamname;

-- 10) Show the stadium and the number of goals scored in each stadium.
SELECT g.stadium, COUNT(go.player) AS 'Goal Scored'
 FROM game g
 JOIN goal go ON g.id = go.matchid
 GROUP BY g.stadium;

-- 11) For every match involving 'POL', show the matchid, date and the number of goals scored.
SELECT g.id, g.mdate, COUNT(go.player) AS 'Goal Scored'
 FROM game g
 JOIN goal go ON g.id = go.matchid
 WHERE (g.team1 = 'POL' OR g.team2 = 'POL') OR go.teamid = 'POL'
 GROUP BY g.id;

-- 12) For every match where 'GER' scored, show matchid, match date and the number of goals scored by 'GER'
SELECT go.matchid, g.mdate, COUNT(go.player) AS 'Goal Scored'
 FROM goal go
...
SELECT g.mdate, g.team1,
    ...
    SUM(CASE
         ...
         END) score2
FROM game g
LEFT JOIN goal go ON go.matchid = g.id
GROUP BY g.id
